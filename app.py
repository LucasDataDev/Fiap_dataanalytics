...
st.write('### vamos coletar as informações necessárias')
...
st.write("### Idade")
input_idade = float(st.slider('Digite sua idade',18,90))

st.write("### Altura")
input_altura = st.slider('Digite sua altura (m)', min_value=1.0, max_value=2.5, step=0.01)
...
input_historico_familiar = st.radio('Você possui histórico na familia?', 
...
st.write("### Consumo alimentos caloricos")
input_freq_calorica = st.radio('Você controla a ingestão de alimentos caloricos?', 
...
st.write("### Controle calorico")
input_controle_calorias = st.radio('Você controla a quantidade de calorias ingeridas?', 
...
st.write("### Habilito de fumar")
input_fuma = st.radio('Voce possui habito de fumar?', options=list(fuma.keys()))
...
st.write("### Álcool")
input_beber = st.selectbox("Você tem o hábito de beber?",options=list(habito_beber.keys()))
...
input_locomover = st.radio("Como voce se locomove?",options=list(locomocao.keys()))
